...
class HFTextGenerator:
    ...
    def generate_stream(self, text: str, **kwargs: Any) -> Iterator[str]:
        ...
        if not isinstance(text, str):
            raise ValueError(
                "Стриминг поддерживает только одиночные строки (batch_size=1)."
            )

        device = next(self.model.parameters()).device

        inputs = self.tokenizer(
            text, return_tensors="pt", padding=True, truncation=True, max_length=self.tokenizer.model_max_length,
        ).to(device)
